...
async function getHomeStats() {
  ...
  try {
    ...
    const [profRes, bookRes, revRes] = await Promise.race([statsPromise, timeoutPromise]);
...
async function authenticateUnified(login, password) {
  ...
  try {
    ...
    if (cMatch && cMatch.password_hash && bcrypt.compareSync(password, cMatch.password_hash)) {
      return {
        id: cMatch.id,
    ...
    if (pMatch && pMatch.password_hash && bcrypt.compareSync(password, pMatch.password_hash)) {
      return {
        id: pMatch.id,
        ...
        onboarding_complete: Boolean(pMatch.onboarding_complete),
        role: "partner"
      };
    }
    ...
    if (proMatch && proMatch.password_hash && bcrypt.compareSync(password, proMatch.password_hash)) {
      return {
        id: proMatch.id,
        ...
        onboarding_complete: Boolean(proMatch.onboarding_complete)
      };
    }

    const localAdmins = await getLocalAdmins();
    const aMatch = localAdmins.find(a => a.email && a.email.toLowerCase() === inputLower);
    if (aMatch && aMatch.password_hash && bcrypt.compareSync(password, aMatch.password_hash)) {
      return {
        id: aMatch.id,
  ...
  // 2. PARALLEL SUPABASE QUERY WITH FAST TIMEOUT
  const dbClient = getClient();
  if (!dbClient) return null;

  try {
    const isEmail = inputLower.includes("@");
    const isPhone = Boolean(input.match(/^\+?[0-9\-\s]{10,20}$/));

    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout")), 2000));

    const adminQuery = isEmail ? dbClient.from("admins").select("*").eq("email", inputLower).maybeSingle() : Promise.resolve({ data: null });
    
    let clientQuery = dbClient.from("clients").select("*");
    if (isEmail) clientQuery = clientQuery.eq("email", inputLower);
    else if (isPhone) clientQuery = clientQuery.eq("phone", input);
    else clientQuery = clientQuery.eq("username", input);

    let partnerQuery = dbClient.from("partners").select("*");
    if (isEmail) partnerQuery = partnerQuery.eq("email", inputLower);
    else if (isPhone) partnerQuery = partnerQuery.eq("phone", input);
    else partnerQuery = partnerQuery.eq("username", input);

    let proQuery = dbClient.from("professionals").select("*");
    if (isEmail) proQuery = proQuery.eq("email", inputLower);
    else if (isPhone) proQuery = proQuery.eq("phone", input);
    else proQuery = proQuery.eq("username", input);

    const [adminRes, clientRes, partnerRes, proRes] = await Promise.race([
      Promise.all([
        adminQuery.catch(() => ({ data: null })),
        clientQuery.maybeSingle().catch(() => ({ data: null })),
        partnerQuery.maybeSingle().catch(() => ({ data: null })),
        proQuery.maybeSingle().catch(() => ({ data: null }))
      ]),
      timeoutPromise.catch(() => [{ data: null }, { data: null }, { data: null }, { data: null }])
    ]);

    if (adminRes && adminRes.data && adminRes.data.password_hash) {
      if (bcrypt.compareSync(password, adminRes.data.password_hash)) {
        return { id: adminRes.data.id, name: adminRes.data.full_name, email: adminRes.data.email, role: "admin" };
      }
    }

    if (clientRes && clientRes.data && clientRes.data.password_hash) {
      if (bcrypt.compareSync(password, clientRes.data.password_hash)) {
        const c = clientRes.data;
        return {
          id: c.id, name: c.full_name || c.name, email: c.email, phone: c.phone, city: c.city,
          username: c.username || "", address: c.address || "", area: c.area || "", pincode: c.pincode || "",
          onboarding_complete: Boolean(c.onboarding_complete || (c.full_name && c.phone && c.city)),
          work_credits: c.work_credits !== undefined ? Number(c.work_credits) : 1, role: "client"
        };
      }
    }

    if (partnerRes && partnerRes.data && partnerRes.data.password_hash) {
      if (bcrypt.compareSync(password, partnerRes.data.password_hash)) {
        const p = partnerRes.data;
        return {
          id: p.id, name: p.full_name || p.name, email: p.email, phone: p.phone, city: p.city,
          username: p.username || "", isVerified: p.is_verified || p.isVerified,
          onboarding_complete: Boolean(p.onboarding_complete), role: "partner"
        };
      }
    }

    if (proRes && proRes.data && proRes.data.password_hash) {
      if (bcrypt.compareSync(password, proRes.data.password_hash)) {
        const pr = proRes.data;
        return {
          id: pr.id, name: pr.full_name || pr.name, email: pr.email, phone: pr.phone, city: pr.city,
          area: pr.area, username: pr.username || "", isVerified: pr.is_verified || pr.isVerified,
          role: "professional", partnerId: pr.partner_id || pr.partnerId,
          isPartnerManaged: pr.is_partner_managed || pr.isPartnerManaged,
          onboarding_complete: Boolean(pr.onboarding_complete)
        };
      }
    }
  } catch (err) {
    console.error("Unified auth check error:", err.message);
  }
