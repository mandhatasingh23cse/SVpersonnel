...
function parseProfessionalMetadata(bio, fallbackHourlyRate, fallbackStartingPrice) {
  ...
  let assignedSkills = [];
  const assignedMatch = rawDesc.match(/\[assignedSkillsJson:([\s\S]*?)\]\]/i);
    ...
    if (clientRow && clientRow.password_hash) {
      if (bcrypt.compareSync(password, clientRow.password_hash)) {
        return {
          ...
          onboarding_complete: Boolean(clientRow.onboarding_complete || (clientRow.full_name && clientRow.phone && clientRow.city)),
          work_credits: clientRow.work_credits !== undefined ? Number(clientRow.work_credits) : 1,
          role: "client"
        };
      }
    }
    ...
    if (partnerRow && partnerRow.password_hash) {
      if (bcrypt.compareSync(password, partnerRow.password_hash)) {
        return {
          ...
          onboarding_complete: Boolean(partnerRow.onboarding_complete),
          role: "partner"
        };
      }
    }
