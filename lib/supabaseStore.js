...
async function getAllClients(cityFilter = null) {
  ...
  try {
    if (isDatabaseReady()) {
      const client = getClient();
      if (client) {
        let query = client.from("clients").select("id, full_name, email, phone, city, created_at, total_works_posted").order("created_at", { ascending: false });
  ...
  if (!clients.length) {
    ...
    if (cityFilter && cityFilter.trim()) {
      ...
      clients = clients.filter(c => (c.city || "").toLowerCase().includes(target));
    }
  }
...
async function getAllPartners() {
  ...
  try {
    if (isDatabaseReady()) {
      const client = getClient();
      if (client) {
        const { data, error } = await client.from("partners").select("id, full_name, business_name, email, phone, city, address, pincode, area, is_verified, created_at").order("created_at", { ascending: false });
        if (!error && Array.isArray(data) && data.length > 0) {
          partners = data.map(p => ({
            id: p.id,
