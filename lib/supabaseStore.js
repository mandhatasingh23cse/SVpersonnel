...
if (!fsSync.existsSync(DATA_CHAT_FILE)) {
  if (!fsSync.existsSync(path.dirname(DATA_CHAT_FILE))) {
    fsSync.mkdirSync(path.dirname(DATA_CHAT_FILE), { recursive: true });
  }
  fsSync.writeFileSync(DATA_CHAT_FILE, JSON.stringify([], null, 2), "utf8");
}
