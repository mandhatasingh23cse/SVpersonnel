...
async function resetUserPassword({ role, email, phone, newPassword }) {
  ...
  const targetRole = String(role || "client").trim().toLowerCase();
  const table = targetRole === "client" ? "clients" : (targetRole === "partner" ? "partners" : "professionals");
  ...
  try {
    if (dbClient) {
      const { data, error } = await dbClient.from(table).select("*").ilike("email", targetEmail).maybeSingle();
      if (!error && data) {
        userRow = data;
      }
    }
  } catch (e) {
    console.warn("Supabase query error in resetUserPassword:", e.message);
  }

  // 2. Try querying local JSON store if DB query returned nothing
  if (!userRow) {
    if (targetRole === "client") {
      const clients = await getLocalClients();
      userRow = clients.find(c => c.email && c.email.toLowerCase() === targetEmail);
    } else if (targetRole === "partner") {
      const partners = await getLocalPartners();
      userRow = partners.find(p => p.email && p.email.toLowerCase() === targetEmail);
    } else {
      const profs = await getLocalProfessionals();
      userRow = profs.find(p => p.email && p.email.toLowerCase() === targetEmail);
    }
  }

  // 3. If account not found in chosen role, check if email exists under a DIFFERENT role!
  if (!userRow) {
    let altRole = null;
    let altRow = null;
    
    if (dbClient) {
      for (const checkRole of ["clients", "partners", "professionals"]) {
        if (checkRole !== table) {
          try {
            const { data } = await dbClient.from(checkRole).select("*").ilike("email", targetEmail).maybeSingle();
            if (data) {
              altRole = checkRole === "clients" ? "client" : (checkRole === "partners" ? "partner" : "professional");
              altRow = data;
              break;
            }
          } catch(e) {}
    ...
    if (altRow) {
      const labelMap = { client: 'Client account', partner: 'Partner Agency account', professional: 'Professional profile' };
      const error = new Error(`This email is registered under a ${altRole.toUpperCase()} account, not a ${targetRole.toUpperCase()} account. Please select "${labelMap[altRole]}" in Account type.`);
      error.statusCode = 400;
      throw error;
    }

    const error = new Error("No registered account found matching that email address.");
    error.statusCode = 404;
    throw error;
  }

  // 4. Validate phone number if both exist
  const cleanInputPhone = String(phone || "").replace(/\D/g, "");
  const cleanRowPhone = String(userRow.phone || "").replace(/\D/g, "");
  if (cleanRowPhone && cleanInputPhone && cleanRowPhone.length >= 10 && cleanInputPhone.length >= 10) {
    if (cleanRowPhone.slice(-10) !== cleanInputPhone.slice(-10)) {
      const error = new Error("The registered phone number does not match our records for this account.");
      error.statusCode = 400;
      throw error;
    }
  }
  ...
  // 6. Update password in Supabase DB (by email AND by ID)
  if (dbClient) {
    try {
      const updatePayload = {
        password_hash: passwordHash,
        ...(userRow.supabase_uid ? { supabase_uid: userRow.supabase_uid } : {})
      };

      const { error: dbErr1 } = await dbClient.from(table).update(updatePayload).ilike("email", targetEmail);
      if (dbErr1) console.error("Error updating password by email in Supabase DB:", dbErr1.message);

      if (userRow.id) {
        const { error: dbErr2 } = await dbClient.from(table).update(updatePayload).eq("id", userRow.id);
        if (dbErr2) console.error("Error updating password by ID in Supabase DB:", dbErr2.message);
      }
    } catch (e) {
      console.error("Failed to update password in Supabase DB:", e.message);
    }

    // Update Supabase Auth User password if available
    try {
      let supabaseUid = userRow.supabase_uid;
      if (!supabaseUid) {
        const { data: listData } = await dbClient.auth.admin.listUsers();
        const existingAuthUser = listData?.users?.find(u => u.email && u.email.toLowerCase() === targetEmail);
        if (existingAuthUser) supabaseUid = existingAuthUser.id;
      }
      if (supabaseUid) {
        await dbClient.auth.admin.updateUserById(supabaseUid, { password: newPassword });
      }
    } catch (e) {}
  }

  // 7. Update local JSON file & sync record so local auth checks pass instantly!
  try {
    const file = targetRole === "client" ? DATA_CLIENTS_FILE : (targetRole === "partner" ? DATA_PARTNERS_FILE : DATA_PROFESSIONALS_FILE);
    let list = [];
    try {
    ...
    let updated = false;
    list = list.map(item => {
      if ((item.email && item.email.toLowerCase() === targetEmail) || (item.id && userRow.id && String(item.id) === String(userRow.id))) {
        updated = true;
        return {
          ...item,
          password_hash: passwordHash
    ...
    console.error("Local store password update notice:", e.message);
  }
