...
async function getClientTotalPostedCount(clientId, clientEmail, userObj) {
  ...
  try {
    if (fsSync.existsSync(DATA_POSTED_WORK_HISTORY_FILE)) {
      ...
      historyCount = history.filter(h => 
        (idStr && String(h.clientId).toLowerCase().trim() === idStr) ||
        (emailStr && String(h.clientId).toLowerCase().trim() === emailStr)
      ).length;
    }
  } catch(e) {}
  ...
  try {
    const active = await getWorkRequirementsByClient(clientId || clientEmail);
    activeJobsCount = active ? active.length : 0;
  } catch(e) {}

  return Math.max(historyCount, userCount, localRecordCount, activeJobsCount);
}
