...
async function seedDatabase() {
  ...
  // Seed default professionals and reviews
  try {
    const { data: existingPros } = await client.from("professionals").select("id, email");
    const existingEmails = new Set((existingPros || []).map((p) => p.email));

    const { data: dbServices } = await client.from("services").select("id, slug");
    const serviceMap = new Map((dbServices || []).map((s) => [s.slug, s.id]));

    if (existingEmails.size === 0 && seedData.professionals) {
      for (const pro of seedData.professionals) {
        if (existingEmails.has(pro.email)) continue;

        const passwordHash = await bcrypt.hash(pro.password, 10);
        const { data: newPro, error: proError } = await client
          .from("professionals")
          .insert({
            ...
            hourly_rate_inr: pro.hourlyRateInr,
        ...
        if (proError) {
          console.warn(`Failed to seed professional ${pro.fullName}: ${proError.message}`);
          continue;
        }

        // Map services
        for (const slug of pro.serviceSlugs) {
          const serviceId = serviceMap.get(slug);
          if (serviceId) {
            const customRate = pro.customRates?.[slug] || null;
            await client.from("professional_services").insert({
              professional_id: newPro.id,
              service_id: serviceId,
              custom_rate_inr: customRate
            });
          }
        }

        // Seed a couple of reviews for this professional
        const sampleReviews = [
          {
            rating: 5,
            review_text: `Excellent service! ${pro.fullName} arrived on time, was very professional, and solved the issue quickly. Highly recommend!`,
            reviewer_name: "Rahul Khanna"
          },
          {
            rating: 4,
            review_text: `Very polite and competent. Did a great job with the setup. Will book again.`,
    ...
    console.warn(`Professionals/reviews seeding failed: ${err.message}`);
  }
}
