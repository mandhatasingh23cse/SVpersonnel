...
async function resetUserPassword({ role, email, phone, newPassword }) {
  const dbClient = getClient();
  const table = role === "client" ? "clients" : (role === "partner" ? "partners" : "professionals");
  let userRow = null;

  try {
    if (dbClient) {
      let query = dbClient.from(table).select("*").eq("email", email.toLowerCase());
  ...
  if (!userRow) {
    if (role === "client") {
      ...
      userRow = clients.find(c => c.email && c.email.toLowerCase() === email.toLowerCase());
    } else if (role === "partner") {
      const partners = await getLocalPartners();
      userRow = partners.find(p => p.email && p.email.toLowerCase() === email.toLowerCase());
    } else {
      const profs = await getLocalProfessionals();
      userRow = profs.find(p => p.email && p.email.toLowerCase() === email.toLowerCase());
    }
  }

  if (!userRow) {
    const error = new Error("No account matches that email address.");
    error.statusCode = 404;
    throw error;
  }
  ...
  if (cleanRowPhone && cleanInputPhone && cleanRowPhone !== cleanInputPhone) {
    const error = new Error("The registered phone number does not match our records.");
    error.statusCode = 400;
    throw error;
  }
    ...
    try {
      if (!supabaseUid) {
        ...
        if (existingAuthUser) {
          ...
          const { data: newAuthUser } = await dbClient.auth.admin.createUser({
            email,
      ...
      if (supabaseUid) {
        await dbClient.auth.admin.updateUserById(supabaseUid, {
          password: newPassword
        });
      }
    } catch (e) {}
  ...
  try {
    const file = role === "client" ? DATA_CLIENTS_FILE : (role === "partner" ? DATA_PARTNERS_FILE : DATA_PROFESSIONALS_FILE);
    const data = await fs.readFile(file, "utf8");
    let list = JSON.parse(data);
    let updated = false;
    list = list.map(item => {
      if ((item.id && item.id === userRow.id) || (item.email && item.email.toLowerCase() === email.toLowerCase())) {
        updated = true;
        return {
          ...item,
          password_hash: passwordHash,
