...
async function getAddressesByClient(clientIdOrEmail) {
  ...
  const localMatching = local.filter(a => String(a.clientId).toLowerCase() === target);
  
  // Combine unique by ID
  const map = new Map();
  list.forEach(a => map.set(String(a.id), a));
  localMatching.forEach(a => map.set(String(a.id), a));
...
async function addClientAddress(clientIdOrEmail, addressData) {
  ...
  try {
    ...
    if (client) {
      ...
      if (error) {
        console.warn("Supabase client_addresses insert note:", error.message);
      }
    }
  } catch (e) {
    console.warn("Supabase client_addresses insert exception:", e.message);
  }
