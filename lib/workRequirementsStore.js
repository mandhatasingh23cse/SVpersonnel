...
async function getAllWorkRequirements() {
  ...
  for (const job of localJobs) {
    if (job && job.id && !deletedSet.has(String(job.id))) map.set(String(job.id), job);
  }
  ...
  const merged = Array.from(map.values()).filter(j => j && j.id && !deletedSet.has(String(j.id)));
  merged.forEach(job => {
...
async function deleteWorkRequirement(workId, clientIdOrEmail) {
  ...
  const local = await getLocalData();
  const targetJob = local.find(w => String(w.id) === wid);

  try {
    const client = getClient();
    if (client) {
      const nid = Number(wid);
      if (!Number.isNaN(nid)) {
  ...
  const filtered = local.filter(w => String(w.id) !== wid && (!targetJob || w.description !== targetJob.description));
  await saveLocalData(filtered);

  if (targetJob && targetJob.id) {
    await addDeletedId(targetJob.id);
  }
  return true;
}
